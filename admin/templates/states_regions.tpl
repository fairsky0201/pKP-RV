...
<h1>State regions<input type="button" value="Add" onclick="modifyItem(0)" /></h1>
...
<tr bgcolor="#000000" style="color:#FFF">
	<th align="center">#</th>
	<th align="left" width="100%">State</th>
...
<tr bgcolor="{{cycle values='#EEE,#CCC'}}">
	<td>{{$n.Id}}</td>
	<td>{{foreach from=$states item=s}}{{if $n.id_state==$s.Id}}<strong>{{$s.name}}</strong>{{/if}}{{/foreach}}</td>
	<td nowrap="nowrap"><strong>{{$n.min}} > {{$n.max}}</strong></td>
...
<tr>
	...
	<td width="100%" colspan="3"><select name="id_state">
...
<tr>
	...
	<td width="50%"><input type="text" name="min" value="0" /></td>
	<td nowrap="nowrap"><strong>To:</strong></td>
	<td width="50%"><input type="text" name="max" value="0" /></td>
</tr>
<tr>
...
<tr>
    ...
    <option value="0">--none--</option>
    {{foreach from=$states item=s}}
    <option value="{{$s.Id}}"{{if $n.id_state==$s.Id}} selected="selected"{{/if}}>{{$s.name}}</option>
    {{/foreach}}
    </select></td>
</tr>
<tr>
	<td nowrap="nowrap"><strong>From:</strong></td>
	<td width="50%"><input type="text" name="min" value="{{$n.min}}" /></td>
	<td nowrap="nowrap"><strong>To:</strong></td>
	<td width="50%"><input type="text" name="max" value="{{$n.max}}" /></td>
</tr>
<tr>
	<td align="center" colspan="4"><input type="submit" value="Save" /> <input type="button" value="Cancel" onclick="showItemListing()" /></td>
</tr>
