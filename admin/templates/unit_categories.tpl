...
<h1>Unit categories <input type="button" value="Add" onclick="modifyItem(0)" /></h1>
...
<tr bgcolor="#000000" style="color:#FFF">
	...
	<th align="center" nowrap>Furniture type</th>
...
<tr bgcolor="{{cycle values='#EEE,#CCC'}}">
	...
	<td align="center" nowrap="nowrap">{{$n.FurnitureType|default:'-'}}</td>
	<td align="center" nowrap="nowrap">{{$n.Parent|default:'-'}}</td>
...
<tr>
	<td nowrap="nowrap"><strong>Furniture type:</strong></td>
	<td><select name="IdFurnitureType">
    <option value="0">none</option>
    {{foreach $furnituretypes as $f}}
    <option value="{{$f.Id}}">{{$f.Name}}</option>
    {{/foreach}}
    </select></td>
</tr>
<tr>
	<td nowrap="nowrap"><strong>Parent category:</strong></td>
	<td><select name="IdParent">
    <option value="0">none</option>
    {{foreach $pcategs as $c}}
...
<tr>
    ...
    <option value="0">none</option>
    {{foreach $furnituretypes as $f}}
    <option value="{{$f.Id}}"{{if $n.IdFurnitureType==$f.Id}} selected="selected"{{/if}}>{{$f.Name}}</option>
    {{/foreach}}
    </select></td>
</tr>
<tr>
    ...
    <option value="0">none</option>
    {{foreach $pcategs as $c}}
    <option value="{{$c.Id}}"{{if $n.IdParent==$c.Id}} selected="selected"{{/if}}>{{$c.Name}}</option>
    {{/foreach}}
    </select></td>
</tr>
<tr>
