<?php
	...
	if( !empty( $_POST['action'] ) && $_POST['action'] == 'modify' )
	{
		...
		{
			$uploader = new image_uploader($_FILES['Picture'],$config['server_path'].'uploads/latches/');
			//$uploader->resize(640,480);
			if( $uploader->error === false )
			{
				if( $_POST['Id'] != -1 && is_file($config['server_path'].'uploads/latches/'.$item['Picture']) )
				{
					unlink($config['server_path'].'uploads/latches/'.$item['Picture']);
				}
	...
	$list = new DbList('select count(distinct id) from latches '.$where);
	$nr = $list[0][0];
	...
	$list = new DbList('select * from latches '.$where.' order by Id DESC limit '.$fl.','.$per_page);
	$items = array();
	foreach( $list as $l )
	{
