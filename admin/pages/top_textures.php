<?php
	...
	{
		$item = new DbTopTexture($_POST['Id']);
		...
		{
			$uploader = new image_uploader($_FILES['Picture'],$config['server_path'].'uploads/top_textures/');
			//$uploader->resize(640,480);
			if( $uploader->error === false )
			{
				if( $_POST['Id'] != -1 && is_file($config['server_path'].'uploads/top_textures/'.$item['Picture']) )
				{
					unlink($config['server_path'].'uploads/top_textures/'.$item['Picture']);
				}
	...
	$list = new DbList('select count(distinct id) from tops '.$where);
	$nr = $list[0][0];
	...
	$list = new DbList('select * from tops '.$where.' order by Id DESC limit '.$fl.','.$per_page);
	$items = array();
	foreach( $list as $l )
	{
	...
	$core['template'] = "top_textures.tpl";
?>