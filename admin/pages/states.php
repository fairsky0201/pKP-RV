<?php
	...
	$list = new DbList('select count(distinct Id) from states '.$where);
	$nr = $list[0][0];
	...
	$list = new DbList('select * from states '.$where.' order by `name` ASC limit '.$fl.','.$per_page);
	$items = $list;
	$smarty->assign('items',$list);
	$smarty->assign('linkwcp',Util::GetCurrentUrlWithoutKeys(array('pg')));
	
	$core['template'] = "states.tpl";
?>