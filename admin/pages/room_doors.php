<?php
	...
	if( !empty( $_POST['action'] ) && $_POST['action'] == 'modify' )
	{
		...
		for( $i = 0; $i< count($_POST['dv_Id']); $i++ )
		{
			...
			$item = new DbRoomDoorPic($_POST['dv_Id'][$i]);
			$found_pics[] = $_POST['dv_Id'][$i] == -1 ? $item->getNextId() : $_POST['dv_Id'][$i];
			$item['IdDoor'] = $item_id;
			$item['Name'] = $_POST['dv_Name'][$i];
			if( !empty( $_FILES['dv_Picture']['name'][$i] ) )
			{
				$uploader = new image_uploader($_FILES['dv_Picture'],$config['server_path'].'uploads/door_variation/',false,$i);
				//$uploader->resize(640,480);
				if( $uploader->error === false )
				{
					if( $_POST['dv_Id'][$i] != -1 && is_file($config['server_path'].'uploads/door_variation/'.$item['Picture']) )
					{
						unlink($config['server_path'].'uploads/door_variation/'.$item['Picture']);
					}
			...
			for( $j = 0; $j<4; $j++ )
			{
				...
				{
					$uploader = new image_uploader($_FILES['dv_Render'.$c],$config['server_path'].'uploads/door_variation/',false,$i);
					//$uploader->resize(640,480);
					if( $uploader->error === false )
					{
						if( $_POST['dv_Id'][$i] != -1 && is_file($config['server_path'].'uploads/door_variation/'.$item['Render'.$c]) )
						{
							unlink($config['server_path'].'uploads/door_variation/'.$item['Render'.$c]);
						}
		...
		if( $_POST['Id'] != -1 )
		{
			$list = new DbList('select Id from room_doors_pics where IdDoor = "'.$item_id.'"');
	...
	$list = new DbList('select count(distinct r.id) from room_doors r, room_doors_types t '.$where);
	$nr = $list[0][0];
	...
	$list = new DbList('select r.* from room_doors r, room_doors_types t '.$where.' order by r.Id DESC limit '.$fl.','.$per_page);
	$items = array();
	foreach( $list as $l )
	{
		$l['Pics'] = new DbList('select * from room_doors_pics where IdDoor="'.$l['Id'].'" order by Id ASC');
		array_push($items,$l);
	}
	$smarty->assign('items',$items);
	$smarty->assign('linkwcp',Util::GetCurrentUrlWithoutKeys(array('pg')));
	
	$core['template'] = "room_doors.tpl";
?>